...
Policy: Target ISIL Income Stream Proposed Policy: \textcolor{red}{Strengthen financial constraints and re-evaluate targeting}  

\subsubsection{????????? Proposed Policy missing ??????????}

Proposed Policy: \textcolor{red}{???}

Lead Agency: Department of the Treasury

Contributing Agency: Intelligence Community \normalfont
...
Lead Agency: National Security Agency

Contributing Agency: Intelligence Community and Department of State \normalfont
...
\subsubsection{?????????  Policy missing ??????????}


Policy: \textcolor{red}{SUMMARY POLICY HERE}

\subsubsection{????????? Proposed Policy missing ??????????}


Proposed Policy: \textcolor{red}{SUMMARY POLICY HERE}
...
While it is extremely unlikely ISIL would attempt an indigenous nuclear program for the reasons detailed in \autoref{sec:fuel_cycle}, if ISIL attempted to develop an indigenous program, airstrikes could easily derail the program. As detailed in \autoref{sec:fuel_cycle}, the fuel for a nuclear weapon requires either large enrichment facilities or a nuclear reactor, depending on the type of fuel used. In either case, the necessary facilities are very large and would be easily detectable by Intelligence, Surveillance, and Reconnaissance operations focusing on Iraq and Syria. 
...
\chapter{???????  Add all ACRONYMS to table ?????}
...
\chapter[Appendix E: ????????]{?????????} \label{app:relations_table}
