...
In international organization system, information sharing would be more complicated than between internal agencies. As for the US, we need to know which countries are in the alliance against ISIL. Definitely, US has different political relationship with each of them. Therefore, we can assume that information sharing level would be according to that political friendship level. Friendship levels can be divided to the three, in simple system. However, in more complicated system of discussion, these levels can be extended until five. \autoref{tab:relation_table} shows the meaning of each level for the friendship in reference with the US:

\subsubsection{??????  Again, the grammar in this table, like most of this section, is almost impossible to even make sense of. This needs to be seriously addressed.   ??????????}
...
1      & Historically and current close relationship. For example, Great Britain, France, Canada.                                                    \\ \hline
2      & \begin{tabular}[c]{@{}l@{}}nor good or bad relations, or last days relationship is decreasing. \\ For example, Germany as neutral [11] and, for decreasing friendship, Israel \cite{Goldberg2014}.\end{tabular} \\ \hline
3      & historically and currently bad relationship. For example, Russia, Jordan, Turkey, Egypt…[10]                                                  \\ \hline
\end{tabular}
\caption{My caption}
\label{tab:relation_table}
\end{table}

\subsubsection{????????  This table needs a caption, reference, update ref [10]  ???????}
