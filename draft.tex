...
\subsection{??????? What section is this referring to ???????????}

\autoref{}, could be modified to limit ISIL's ability to smuggle a nuclear weapon out of its territory \cite{Kilmartin2010}. The same proposed changes to stem the inflow of nuclear materials into ISIL controlled territory would apply here as well. The addition of the Port of Lattakia and the Port of Tartous 

\subsection{???????? No Figure 1 provided  ?????????}

(see Figure 1) reduces the threat of overseas delivery of a nuclear weapon.   
